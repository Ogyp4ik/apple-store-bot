...
// ID группы для уведомлений (получим позже)
const GROUP_CHAT_ID = -1003850642883; // твоя группа "Заказы"
...
async function setPersistentMenu() {
    try {
        ...
        console.error('❌ Ошибка установки кнопки:', error.message);
    }
}
...
async function setAdminCommands() {
    ...
    for (const adminId of ADMIN_IDS) {
        try {
            ...
            console.error(`❌ Ошибка для админа ${adminId}:`, error.message);
        }
    }
}
...
async function watchOrders() {
    console.log('👀 Начинаем отслеживание заказов...');
...
📅 Время: ${date}`;
                ...
                // Отправляем в группу (просто текст, без кнопок)
                if (GROUP_CHAT_ID) {
...
// Команда /start (только в личке, не в группах)
bot.start(async (ctx) => {
    // Если команда из группы — игнорируем
